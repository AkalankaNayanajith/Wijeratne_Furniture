...
const MainContent = () => {
    ...
    <div className='card border-solid w-full cursor-pointer group flex-col relative transition h-[60vh] rounded-xl ease-in-out hover:scale-105 duration-300 mb-8  hover:bg-slate-700' key={item.id}>
        ...
        <div className='card_header m-4 flex-1 h-1/2 flex flex-col space-y-3 items-center'>
            ...
            <p className='price text-[#fe8033] text-[25px] inline-block align-bottom relative'> {item.price} <span className='absolute -left-4 bottom-[0.37rem] text-xs'> {item.Currency}</span></p>
            <div className='btn text-white'> <button className='w-[175px] h-[35px] bg-slate-800 overflow-hidden transition duration-300 group-hover:bg-[#fe8033] rounded-full relative cursor-pointer '> Add to cart </button> </div>
        </div>
    </div>
);
